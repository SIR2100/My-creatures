...
//=================================================================================================
        ...
        public void StartServer()
        {
            ...
            TypeAddText2Delegate AddText2Delegate = new TypeAddText2Delegate(AddText2);
            try
            {
                ...
                //Загоняем задания из многострочного текстбокса в двумерный массив Z[номер][задание]
                string query = "";
                ...
                while (true)
                {
                    if (server.Pending())//проверяем, есть ли в очереди клиенты
                    {
                     ...
                     //      Invoke(AddText2Delegate, recv_message);
                                   int len = recv_message.Length;
                                   if (len > 15)
                                   {
                                       Invoke(AddText2Delegate, recv_message);
                                   }
                            ...
                            query = textBox2.Text;
                            answer_message = query;  //res.ToString();
                            //перед отправкой чуток подолждем - это мы симулируем задержки в сети
                   //         Thread.Sleep(1000);
                            //печатаем то, что будем отправлять
                            Invoke(AddTextDelegate, "Targets have been sent!" + "\r\n");
                            //преобразуем строчку-ответ сервера в массив байт
                            byte[] answer_bytes = System.Text.Encoding.UTF8.GetBytes(answer_message);
                            // отправляем ответ
...
//=================================================================================================
        ...
        private delegate void TypeAddText2Delegate(string text);
...
//=================================================================================================
        ...
        private void AddText2(string text)
        {
            textBox2.Text = text;
        }
